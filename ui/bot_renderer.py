...
def render_bot_tab():
    ...
    # ── Lazy import engine (avoids import cost when tab not open) ─────────────
    from bot.engine import start_bot, stop_bot, get_state
    from bot.logger import (
        get_recent_trades, get_recent_runs, get_trade_summary_today,
    )
    ...
    st.caption("Powered by Katraswing analysis pipeline · Executes on Alpaca Paper Trading")
    st.divider()
    ...
    pnl_color     = "#00cc66" if daily_pnl >= 0 else "#ff4444"
    mkt_color     = "#00cc66" if market_open else "#ff9900"
    mkt_label     = "OPEN" if market_open else "CLOSED"

    m1, m2, m3, m4, m5, m6 = st.columns(6)
    m1.metric("Market",        mkt_label,         delta=None)
...
| Scan interval | {SCAN_INTERVAL_MINUTES} min |
        """)
        st.caption("Edit `bot/config.py` to change these settings.")
        st.markdown(f"""
| Alpaca Account |   |
|---|---|
| Mode | 🧪 Paper Trading |
